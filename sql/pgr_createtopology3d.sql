-- Function: routing.pgr_createtopology3d(text, double precision, text, text, text, text, text)

-- DROP FUNCTION routing.pgr_createtopology3d(text, double precision, text, text, text, text, text);

CREATE OR REPLACE FUNCTION routing.pgr_createtopology3d(edge_table text, tolerance double precision, the_geom text DEFAULT 'the_geom'::text, id text DEFAULT 'id'::text, source text DEFAULT 'source'::text, target text DEFAULT 'target'::text, rows_where text DEFAULT 'true'::text)
...
$BODY$
  ...
  COST 100;
ALTER FUNCTION routing.pgr_createtopology3d(text, double precision, text, text, text, text, text)
  OWNER TO routing;
COMMENT ON FUNCTION routing.pgr_createtopology3d(text, double precision, text, text, text, text, text) IS 'args: edge_table,tolerance, the_geom:=''the_geom'',source:=''source'', target:=''target'',rows_where:=''true'' - fills columns source and target in the geometry table and creates a vertices table for selected rows';
